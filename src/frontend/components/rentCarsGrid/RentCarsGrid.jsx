...
function RentCarsGrid(rentBrandName, makeWithModels, countCars) {
  const [carsForRent, setCarsForRent] = useState([]);
  const [countCarsForRent, setCountCarsForRent] = useState(0);
  const [loading, setLoading] = useState(true);
  const [contentLaoderCount, setContentLoaderCount] = useState([1, 2, 3, 4]);
              ...
              carsForRent.map((car, index) => (
                <div className="auction-item col-md-3 pb-4" key={index}>
                  ...
                  >
                    <Link
                      className="hero"
                      to={`/car_rental_overview/${car.stock_number}/${formatUrl(
